...
import UserTable from "@/pages/verifymanagement/userTable";
import Paginator from '@/components/paginator';
import getTotalNum from "@/actions/verifymanagement/getTotalNum";
import getVerifyRequestedUsers from "@/actions/verifymanagement/getVerifyRequestedUsers";
...
const Dashboard = () => {
    ...
    const getData = useCallback(async () => {
        ...
        let datas = typeof status === 'boolean' ? await getVerifyRequestedUsers(pageVal, filter) : await getVerifyRequestedUsers(pageVal);
        setTotalNum(count);
        ...
        <Layout>
            ...
            <section className="w-full p-8">
                {loading ? (
                        <>
                            <div
                                className="flex md:flex-row flex-col text-xl sm:justify-between mb-4 font-bold items-center">
                                <Skeleton width="10%"/>
                                <Skeleton width="30%"/>
                            </div>
                ...
                <span className="w-20 mr-4 whitespace-nowrap flex items-center !text-base">
                  Filter by:&nbsp;&nbsp;
                                ...
                                <MultiSelect
                                    ...
                                    options={filterOptions.is_verified}
                                    optionLabel="name"
                                    placeholder="Is Verifeid?"
                                    maxSelectedLabels={2}
                                    className="w-full select-center"
                                />

                                <button className="check-btn bg-deepback text-white rounded-r-md px-4 text-xl"
                                        onClick={clickFilter}>
                                    <CgArrowRight/>
                                </button>
                            </div>
                ...
                <div
                    className='flex min-[480px]:flex-row flex-col min-[480px]:justify-between justify-items-stretch min-[480px]:items-center mt-4'>
                    {totalNum === 0
                        ?
                        loading ? <>
                            <Skeleton width="20%"/>
                        ...
                        :
                        <>
                            {loading ? <Skeleton width="20%"/> : <span
                                className="min-[480px]:text-md text-sm min-[480px]:mb-0 mb-4">Showing {pageVal.start + 1} - {pageVal.end + 1 > totalNum ? totalNum : pageVal.end + 1} users of {totalNum}</span>}
